...
growth_long2 <- pivot_longer(data = growth2,
                            ...
                            names_to = "light_level",
                            values_to = "growth")
growth_long2$light_level <- factor(growth_long2$light_level, levels = c("low","med","high"))
head(growth_long2)
```
...
## Caution when interpreting interaction! {.center}
...
-   Even if interaction is not determined statistically significant via an ANOVA, results still need to be looked at with caution
    ...
    -   Individual effects should not be interpreted if there is any kind of interaction, significant or not.
...
This interaction plot makes it hard to tell if there is interaction between fertilizer and light level.

## Creating an Interaction Plot {.center}

Often, it is helpful to switch the axes. Now, we clearly see some interaction between the two factors.

```{r}
#| fig-align: center
interaction.plot(x.factor = growth_long2$light_level,
                 trace.factor = growth_long2$fertilizer, 
                 response = growth_long2$growth, fun = mean,
                 xlab = "Light level", ylab = "Growth (cm)", trace.label = "Fertilizer",
                 col = c("#CC79A7","#0072B2"), lty = 1)
```
...
-   Tukey's HSD test can give great further insight, especially when 
ANOVA results in a statistically insignificant interaction.
