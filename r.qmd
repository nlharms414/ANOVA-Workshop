...
growth <- data.frame(plant = c("plant 1","plant 2","plant 3","plant 4","plant 5","plant 6"),
                     low = c(18.51,5.99,14.02,7.06,14.96,13.56),
...
This interaction plot shows little to no interaction between fertilizer and light level, which was confimed by the ANOVA.

## Tukey's HSD Test for Two-Way ANOVA {.center}

Here, I've only printed the `TukeyHSD()` output for the light level and fertilizer interaction. 

```{r}
TukeyHSD(model2, conf.level = 0.95)$`light_level:fertilizer`
```
...
-   If you are ever worried that variances between groups 
